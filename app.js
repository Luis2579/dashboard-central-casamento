...
function render(){
  ...
  renderAlerts();
  const renderers={home:renderHome,finance:renderFinance,suppliers:renderSuppliers,guests:renderGuests,rsvp:renderRsvp,tables:renderTables,checklist:renderChecklist,timeline:renderTimeline,contracts:renderContracts,decor:renderDecor,gifts:renderGifts,reports:renderReports,music:()=>renderGeneric("Música")};
...
function moduleTransferBar(view){
  ...
  if(["finance","suppliers","guests","rsvp","tables","checklist","timeline","contracts","decor"].includes(view))return "";
...
function renderDecor(){
  const areas=[...new Set(["Cerimônia","Mesa dos convidados","Mesa do bolo","Lounge","Entrada","Papelaria","Convite","Identidade visual",...state.decor.map(d=>d.area).filter(Boolean)])];
  return `<div class="toolbar"><button class="button secondary" data-module-template="decor">Modelo</button><button class="button secondary" data-module-import="decor">Importar</button><button class="button secondary" data-module-export="decor">Exportar</button><button class="button primary" data-action="new-decor">+ Decisão Visual</button></div><div class="decor-areas">${areas.map(area=>{const rows=state.decor.filter(d=>d.area===area);return `<section class="decor-area"><div class="decor-area-heading"><h2>${area}</h2><span>${rows.length}</span></div><div class="decor-items">${rows.map(d=>decorCard(d)).join("")||`<p class="decor-empty">Nenhuma decisão registrada.</p>`}</div><button class="mini-button decor-add" data-action="new-decor" data-area="${area}">+ adicionar</button></section>`}).join("")}</div>`;
}
function isImageFile(item){return /\.(png|jpe?g|webp|gif|avif)$/i.test(item.documentName||item.documentPath||item.documentUrl||"")}
function decorCard(d){const hasFile=d.documentPath||d.documentUrl,link=d.attachment||d.reference||"",hasLink=/^https?:\/\//i.test(link);return `<article class="decor-item-card"><div class="decor-item-copy"><div class="record-card-top"><h3>${esc(d.item)}</h3>${status(d.status)}</div><p>${esc(d.palette||"Paleta não informada")} · ${esc(d.style||"Estilo não informado")}</p><p>${esc(d.notes||"Sem observações")}</p><div class="row-actions">${hasFile?`<button class="mini-button" data-open-decor="${d.id}">Abrir anexo</button>`:""}${hasLink?`<a class="mini-button" href="${esc(link)}" target="_blank" rel="noopener">Abrir link</a>`:""}<button class="mini-button" data-edit="decor:${d.id}">Editar</button><button class="mini-button" data-delete="decor:${d.id}">Excluir</button></div></div>${hasFile?isImageFile(d)?`<div class="decor-item-media"><img data-decor-image="${d.id}" alt="${esc(d.item)}"></div>`:`<button class="decor-file-tile" data-open-decor="${d.id}"><span>Arquivo anexado</span><strong>${esc(d.documentName||"Abrir referência")}</strong></button>`:`<div class="decor-item-placeholder"><span>Sem imagem</span></div>`}</article>`}
