...
function renderDecor(){
  ...
  return `<div class="toolbar"><button class="button secondary" data-module-template="decor">Modelo</button><button class="button secondary" data-module-import="decor">Importar</button><button class="button secondary" data-module-export="decor">Exportar</button><button class="button primary" data-action="new-decor">+ Decisão Visual</button></div><div class="cards-list decor-area-grid">${areas.map(area=>{const rows=state.decor.filter(d=>d.area===area);return `<article class="record-card decor-area-card"><div class="record-card-top"><h3>${area}</h3><span>${rows.length}</span></div><div class="decor-row-list">${rows.map(d=>decorRow(d)).join("")||`<p class="decor-empty">Nenhuma decisão registrada.</p>`}</div><button class="mini-button decor-add" data-action="new-decor" data-area="${area}">+ adicionar</button></article>`}).join("")}</div>`;
}
