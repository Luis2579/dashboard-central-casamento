...
const MODULES = [
  ...
  {id:"buffet",section:"Experiência",icon:"B",title:"Buffet e bebidas",description:"Cardápio, quantidades e custo por convidado."},
  {id:"decor",section:"Experiência",icon:"D",title:"Decoração e identidade",description:"Paleta, referências, áreas e decisões visuais."},
  {id:"music",section:"Experiência",icon:"M",title:"Música e atrações",description:"Playlist, banda, DJ e momentos musicais."},
...
function migrateState(raw){
  ...
  next.decor=enrich(next.decor,{priority:"Média",attachment:""});
...
function attentionCounts(){
  return {
    ...
    buffet:state.buffet.filter(b=>!["Contratado","Incluído","Concluído"].includes(b.status)).length,
...
function render(){
  ...
  renderAlerts();
  const renderers={home:renderHome,finance:renderFinance,suppliers:renderSuppliers,guests:renderGuests,rsvp:renderRsvp,tables:renderTables,checklist:renderChecklist,timeline:renderTimeline,contracts:renderContracts,buffet:renderBuffet,decor:renderDecor,gifts:renderGifts,reports:renderReports,music:()=>renderGeneric("Música")};
...
function renderHome(){
  ...
  const mode=state.settings.viewMode||"couple",priority={couple:["reports","finance","checklist","rsvp","guests"],planner:["timeline","suppliers","checklist","contracts","guests"],finance:["finance","contracts","suppliers","buffet","reports"]}[mode];
...
function supplierContent(){
  ...
  if(supplierMode==="cards")return `<div class="cards-list">${rows.map(s=>{const p=state.payments.filter(x=>x.supplierId===s.id),paid=p.filter(x=>x.paid).reduce((a,b)=>a+Number(b.amount),0);return `<article class="record-card"><div class="record-card-top"><h3>${esc(s.name)}</h3>${status(s.status)}</div><p>${esc(s.service)} · ${esc(s.category)}</p><div class="record-meta"><div><span>Contrato</span><strong>${money(s.total)}</strong></div><div><span>Pago</span><strong>${money(paid)}</strong></div><div><span>Contato</span><strong>${esc(s.contact||"—")}</strong></div><div><span>Assinado</span><strong>${s.contractSigned?"Sim":"Não"}</strong></div></div><div class="row-actions"><button class="mini-button" data-action="new-payment" data-supplier="${s.id}">+ cobrança</button><button class="mini-button" data-edit="supplier:${s.id}">Editar</button></div></article>`}).join("")}</div>`;
...
function renderContracts(){
  ...
  const rows=state.contracts.map(c=>{const hasFile=c.documentPath||c.documentUrl;return `<tr><td><strong>${esc(c.name)}</strong></td><td>${esc(supplier(c.supplierId)?.name||"—")}</td><td>${esc(c.type)}</td><td>${dateBR(c.date)}</td><td>${status(c.status)}</td><td>${dateBR(c.deadline)}</td><td>${hasFile?"Arquivo anexado":esc(c.link||"—")}</td><td><div class="row-actions"><button class="mini-button" data-contract-toggle="${c.id}">Detalhes</button>${hasFile?`<button class="mini-button" data-open-document="${c.id}">Abrir</button>`:""}<button class="mini-button" data-edit="contract:${c.id}">Editar</button><button class="mini-button" data-delete="contract:${c.id}">Excluir</button></div></td></tr><tr class="contract-detail-row" data-contract-detail="${c.id}" hidden><td colspan="8"><div class="contract-detail"><div class="contract-detail-block"><span>Documento</span><strong>${esc(c.documentName||"Nenhum arquivo anexado")}</strong>${hasFile?`<button class="document-link" data-open-document="${c.id}">Abrir documento ↗</button>`:""}</div><div class="contract-detail-block"><span>Referência externa</span><strong>${esc(c.link||"Não informada")}</strong></div><div class="contract-detail-block"><span>Prazo importante</span><strong>${dateBR(c.deadline)}</strong></div><div class="contract-detail-block"><span>Observações</span><p>${esc(c.notes||"Sem observações")}</p></div></div></td></tr>`}).join("");
  return `<div class="metric-grid">${metric("Documentos",state.contracts.length,"Itens cadastrados")}${metric("Assinados",state.contracts.filter(c=>c.status==="Assinado").length,"Concluídos")}${metric("Pendentes",unsigned,"Exigem acompanhamento",unsigned?"rose":"")}${metric("Fornecedores sem contrato",state.suppliers.filter(s=>!s.contractSigned&&["Contratado","Pago parcialmente","Pago"].includes(s.status)).length,"Verificar documentação","gold")}${metric("Com arquivo",state.contracts.filter(c=>c.documentPath||c.documentUrl).length,"Documentos anexados")}</div><div class="toolbar"><button class="button primary" data-action="new-contract">+ Documento</button></div><div class="table-card"><table><thead><tr><th>Documento</th><th>Fornecedor</th><th>Tipo</th><th>Data</th><th>Status</th><th>Prazo</th><th>Arquivo / referência</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
...
function renderDecor(){
  const areas=["Cerimônia","Mesa dos convidados","Mesa do bolo","Lounge","Entrada","Papelaria","Convite","Identidade visual"];
  return `<div class="toolbar"><button class="button primary" data-action="new-decor">+ Decisão visual</button></div><div class="cards-list">${areas.map(area=>{const rows=state.decor.filter(d=>d.area===area);return `<article class="record-card"><div class="record-card-top"><h3>${area}</h3><span>${rows.length}</span></div>${rows.map(d=>`<div class="attention-item"><div><strong>${esc(d.item)}</strong><span>${esc(d.palette)} · ${esc(d.style)}</span></div>${status(d.status)}<div class="row-actions"><button class="mini-button" data-edit="decor:${d.id}">Editar</button></div></div>`).join("")||`<p>Nenhuma decisão registrada.</p>`}<button class="mini-button" data-action="new-decor" data-area="${area}">+ adicionar</button></article>`}).join("")}</div>`;
}
...
const FORMS = {
  ...
  decor:{title:"Decisão visual",note:"Organize referências e aprovações por área do casamento.",fields:[
    ["area","Área","options",false,["Cerimônia","Mesa dos convidados","Mesa do bolo","Lounge","Entrada","Papelaria","Convite","Identidade visual"]],["item","Item / decisão","text",true],["palette","Paleta de cores","text"],["style","Estilo","text"],["supplierId","Fornecedor","suppliers"],["reference","Referência visual","text"],["status","Status","options",false,["Planejado","Em aprovação","Aprovado","Pendente","Concluído"]],["priority","Prioridade","priority"],["tags","Tags","tags"],["attachment","Link / anexo","text"],["notes","Observações","textarea",false,"span-2"]]},
...
$("#globalSearch").addEventListener("keydown",event=>{if(event.key!=="Enter")return;const q=normalize(event.target.value);if(!q)return;const results=[
  ...
  ...state.buffet.map(x=>({type:"Buffet",title:x.item,detail:`${x.kind} ${x.status}`,view:"buffet"})),
