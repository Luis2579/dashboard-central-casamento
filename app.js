...
const sampleData = {
  version: APP_VERSION,
  settings: { budget: 180000, weddingDate: "2028-10-08", couple: "Luís Francisco & Maria Júlia", guestTarget: 180, sidebarCollapsed: false, viewMode:"couple", theme:"light", buffetBasis:"confirmed", buffetManualCount:180 },
...
let lastMigrationChanged = false;
...
function removeSampleRecords(next){
  for(const key of ["suppliers","payments","guests","tasks","contracts","timeline","buffet","decor","gifts","plans","tables"]){
    ...
    if(next[key].length!==before)lastMigrationChanged=true;
  }
}
function migrateState(raw){
  lastMigrationChanged = false;
  const next = raw || {};
  ...
  if(next.settings.weddingDate === "2028-10-09"){
    next.settings.weddingDate = "2028-10-08";
    lastMigrationChanged = true;
  }
...
function filterValue(id){return document.getElementById(id)?.value ?? viewFilters[id] ?? ""}
function selectAttr(id,value){return String(filterValue(id))===String(value)?"selected":""}
...
function budgetMetric(){return `<article class="metric gold metric-editable"><div class="metric-label"><span>Orçamento</span><button type="button" class="metric-edit" data-action="edit-budget" aria-label="Editar orçamento">Editar</button></div><strong class="metric-value">${money(state.settings.budget)}</strong><div class="metric-sub">Planejado</div></article>`}
...
function financeRows(filtered=true){
  ...
  const q=normalize(filterValue("financeSearch")),month=filterValue("financeMonth"),category=filterValue("financeCategory"),sup=filterValue("financeSupplier"),method=filterValue("financeMethod"),tag=filterValue("financeTag");
...
function renderFinance(){
  ...
  return `<div class="metric-grid">${budgetMetric()}${metric("Contratado",money(t.contracted),`${(ratio*100).toFixed(1)}% comprometido`,ratio>1?"rose":"")}${metric("Já pago",money(t.paid),`${t.contracted?(t.paid/t.contracted*100).toFixed(1):0}% dos contratos`)}${metric("Em aberto",money(t.pending),"Pagamentos lançados","gold")}${metric("Vencido",money(t.overdue),"Requer atenção",t.overdue?"rose":"")}</div>
  ...
  <div class="toolbar" style="margin-top:14px"><div class="segmented">${["all","pending","overdue","paid"].map((x,i)=>`<button class="${paymentFilter===x?"active":""}" data-payment-filter="${x}">${["Todos","Pendentes","Vencidos","Pagos"][i]}</button>`).join("")}</div><div class="search-wrap"><span>⌕</span><input id="financeSearch" placeholder="Buscar lançamento" value="${esc(filterValue("financeSearch"))}"></div><input type="month" id="financeMonth" value="${esc(filterValue("financeMonth"))}"><select id="financeCategory"><option value="">Categorias</option>${state.categories.map(x=>`<option ${selectAttr("financeCategory",x)}>${esc(x)}</option>`).join("")}</select><select id="financeSupplier"><option value="">Fornecedores</option>${state.suppliers.map(x=>`<option value="${x.id}" ${selectAttr("financeSupplier",x.id)}>${esc(x.name)}</option>`).join("")}</select><select id="financeMethod"><option value="">Formas</option>${["PIX","Boleto","Transferência","Cartão","Dinheiro","Outro"].map(x=>`<option ${selectAttr("financeMethod",x)}>${x}</option>`).join("")}</select><select id="financeTag"><option value="">Tags</option>${tagOptions("payments").map(x=>`<option ${selectAttr("financeTag",x)}>${esc(x)}</option>`).join("")}</select><button class="button danger" data-bulk-delete="payment">Excluir filtrados</button><button class="button secondary" data-action="manage-categories">Categorias</button><button class="button secondary" data-action="download-template">Modelo</button><button class="button secondary" data-action="import-payments">Importar</button><button class="button secondary" data-action="export-finance">Exportar</button><button class="button primary" data-action="new-payment">+ Pagamento</button></div>
...
function renderSuppliers(){
  return `<div class="toolbar"><div class="segmented"><button class="${supplierMode==="table"?"active":""}" data-supplier-mode="table">Tabela</button><button class="${supplierMode==="cards"?"active":""}" data-supplier-mode="cards">Cards</button></div><div class="search-wrap"><span>⌕</span><input id="supplierSearch" placeholder="Buscar fornecedor" value="${esc(filterValue("supplierSearch"))}"></div><select id="supplierCategory"><option value="">Categorias</option>${state.categories.map(x=>`<option ${selectAttr("supplierCategory",x)}>${esc(x)}</option>`).join("")}</select><select id="supplierStatus"><option value="">Todos os status</option>${["Pesquisando","Orçado","Em negociação","Contratado","Pago parcialmente","Pago","Cancelado"].map(x=>`<option ${selectAttr("supplierStatus",x)}>${x}</option>`).join("")}</select><select id="supplierContract"><option value="">Contrato</option><option value="yes" ${selectAttr("supplierContract","yes")}>Assinado</option><option value="no" ${selectAttr("supplierContract","no")}>Sem assinatura</option></select><select id="supplierTag"><option value="">Tags</option>${tagOptions("suppliers").map(x=>`<option ${selectAttr("supplierTag",x)}>${esc(x)}</option>`).join("")}</select><input id="supplierMin" type="number" min="0" placeholder="Valor mínimo" value="${esc(filterValue("supplierMin"))}" style="width:115px;padding:9px;border:1px solid var(--line);background:var(--paper)"><span class="result-count">${supplierRows().length} fornecedores</span><button class="button danger" data-bulk-delete="supplier">Excluir filtrados</button><button class="button secondary" data-action="supplier-template">Modelo</button><button class="button secondary" data-action="import-suppliers">Importar</button><button class="button secondary" data-action="export-suppliers">Exportar</button><button class="button primary" data-action="new-supplier">+ Fornecedor</button></div><div id="supplierContent">${supplierContent()}</div>`;
}
function supplierRows(){
  const q=normalize(filterValue("supplierSearch")),cat=filterValue("supplierCategory"),st=filterValue("supplierStatus"),contract=filterValue("supplierContract"),tag=filterValue("supplierTag"),min=Number(filterValue("supplierMin")||0);
  return state.suppliers.filter(s=>(!q||normalize(`${s.name} ${s.service} ${s.contact}`).includes(q))&&(!cat||s.category===cat)&&(!st||s.status===st)&&(!contract||(contract==="yes")===Boolean(s.contractSigned))&&(!tag||parseTags(s.tags).includes(tag))&&Number(s.total)>=min);
}
...
function renderGuests(){
  ...
  return `<div class="metric-grid">${metric("Lista total",g.total,"Pessoas cadastradas")}${metric("Confirmados",g.confirmed,`${g.total?Math.round(g.confirmed/g.total*100):0}% da lista`)}${metric("Pendentes",g.pending,"Aguardando resposta","gold")}${metric("Recusados",g.declined,"Respostas negativas","rose")}${metric("Custo estimado",money(cost),"Por confirmado")}</div><div class="toolbar"><div class="search-wrap"><span>⌕</span><input id="guestSearch" placeholder="Buscar convidado" value="${esc(filterValue("guestSearch"))}"></div><select id="guestSide"><option value="">Todos os lados</option>${["Noiva","Noivo","Ambos"].map(x=>`<option ${selectAttr("guestSide",x)}>${x}</option>`).join("")}</select><select id="guestStatus"><option value="">Todos os status</option>${["Não enviado","Enviado","Confirmado","Recusado","Talvez"].map(x=>`<option ${selectAttr("guestStatus",x)}>${x}</option>`).join("")}</select><select id="guestType"><option value="">Todos os tipos</option>${["Adulto","Criança","Bebê"].map(x=>`<option ${selectAttr("guestType",x)}>${x}</option>`).join("")}</select><select id="guestTag"><option value="">Tags</option>${tagOptions("guests").map(x=>`<option ${selectAttr("guestTag",x)}>${esc(x)}</option>`).join("")}</select><button class="button danger" data-bulk-delete="guest">Excluir filtrados</button><button class="button secondary" data-action="guest-template">Modelo</button><button class="button secondary" data-action="import-guests">Importar</button><button class="button secondary" data-action="export-guests">Exportar</button><button class="button secondary" data-action="print-guests">Imprimir</button><button class="button primary" data-action="new-guest">+ Convidado</button></div><div id="guestContent">${guestTable()}</div>`;
}
function guestRows(){
  const q=normalize(filterValue("guestSearch")),side=filterValue("guestSide"),st=filterValue("guestStatus"),type=filterValue("guestType"),tag=filterValue("guestTag");
  return state.guests.filter(g=>(!q||normalize(`${g.name} ${g.group} ${g.phone}`).includes(q))&&(!side||g.side===side)&&(!st||g.inviteStatus===st)&&(!type||g.type===type)&&(!tag||parseTags(g.tags).includes(tag)));
}
function guestTable(){
...
function filteredRowsForDelete(type){
  if(type==="payment")return financeRows();
  if(type==="supplier")return supplierRows();
  if(type==="guest")return guestRows();
  if(type==="contract")return contractRows();
  return [];
}
async function bulkDeleteFiltered(type){
  const collection=COLLECTIONS[type],labels={payment:"pagamentos",supplier:"fornecedores",guest:"convidados",contract:"documentos"},rows=filteredRowsForDelete(type);
  if(!collection||!rows.length){toast("Nenhum registro encontrado pelo filtro atual.");return}
  if(!confirm(`Excluir ${rows.length} ${labels[type]||"registros"} filtrados? Esta ação não pode ser desfeita.`))return;
  let targets=rows,skipped=0;
  if(type==="supplier"){
    targets=rows.filter(row=>!state.payments.some(p=>p.supplierId===row.id));
    skipped=rows.length-targets.length;
  }
  if(type==="contract"){
    for(const record of targets){
      if(record.documentPath)try{await window.CloudStore.deleteDocument(record.documentPath)}catch(error){toast(`Não foi possível excluir o arquivo ${record.documentName||record.name}: ${error.message}`);return}
    }
  }
  const ids=new Set(targets.map(row=>row.id));
  state[collection]=state[collection].filter(row=>!ids.has(row.id));
  save(`${targets.length} ${labels[type]||"registros"} excluídos${skipped?` e ${skipped} ignorados por vínculo com pagamentos`:""}.`);
}
function refreshFilteredContent(id){
  if(["financeSearch","financeMonth","financeCategory","financeSupplier","financeMethod","financeTag"].includes(id)){const body=$("#financeRows");if(body)body.innerHTML=paymentTableRows(financeRows());return true}
  if(["supplierSearch","supplierCategory","supplierStatus","supplierContract","supplierMin","supplierTag"].includes(id)){const target=$("#supplierContent");if(target)target.innerHTML=supplierContent();return true}
  if(["guestSearch","guestSide","guestStatus","guestType","guestTag"].includes(id)){const target=$("#guestContent");if(target)target.innerHTML=guestTable();return true}
  if(["timelineOwner","timelineSupplier","timelineStatus"].includes(id)){const target=$("#timelineContent");if(target)target.innerHTML=timelineContent();return true}
  return false;
}
...
document.addEventListener("click",async event=>{
  ...
  const bulkDelete=event.target.closest("[data-bulk-delete]");if(bulkDelete){await bulkDeleteFiltered(bulkDelete.dataset.bulkDelete);return}
...
document.addEventListener("input",event=>{
  if(event.target.id)viewFilters[event.target.id]=event.target.value;
  if(event.target.id==="monthlyRange"){monthlyWindowStart=Number(event.target.value)||0;renderMonthlyChart()}
  refreshFilteredContent(event.target.id);
});
...
$("#budgetForm").addEventListener("submit",event=>{event.preventDefault();const value=Number($("#budgetInput").value);if(!Number.isFinite(value)||value<0){toast("Informe um orçamento válido.");return}state.settings.budget=value;$("#budgetDialog").close();save("Orçamento do casamento atualizado.","Financeiro","Orçamento")});
...
async function initializeCloud(){
  ...
  try{
    ...
    if(remote){
      ...
      shouldSave=lastMigrationChanged;
      render();
    }else{
    ...
    window.CloudStore.subscribe(remoteState=>{if(!remoteState)return;state=migrateState(remoteState);const migrated=lastMigrationChanged;localStorage.setItem(STORAGE_KEY,JSON.stringify(state));render();if(migrated)window.CloudStore.save(state).catch(error=>toast(`Sincronização pendente: ${error.message}`))});
